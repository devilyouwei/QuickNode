...
class Public{
    ...
    static async auth2(user){
        try {
            ...
            if(user.id && user.access_token){
                let data = await db.query('select * from customer where id=? and access_token=?', [user.id,user.access_token])
                if(data.length>0) return data[0] // 用戶存在
                else return false
            } else return false
        } catch(e) {
    ...
    static async post(url,data={},header={}){
        ...
        try{
            let res = await superagent.post(url).type('form').set(header).send(data)
